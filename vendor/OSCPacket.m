...
- (void)addArgument:(id)arg
{
    ...
    [(NSMutableArray *)_arguments addObject:arg];
...
- (NSString *)typeTag
{
    NSMutableString *str = [NSMutableString stringWithCapacity:[_arguments count]+1];
    [str appendString:@","];
...
- (NSData *)encode
{
    ...
    {
        NSLog(@"Failed to encode OSCPacket because address doesn't start with a slash: %@", self.address);
        return nil;
    }
    [data appendData:[self.address oscStringData]];
...
- (NSString *)description
{
    return [NSString stringWithFormat:@"<OSCMutableMessage (%@) %@>", self.address, self.arguments];
}
...
@synthesize childPackets = _childPackets;
...
- (id)initWithData:(NSData *)data
{
    ...
    {
        // Init ivars.
        _childPackets = [[NSMutableArray alloc] init];
        _timetag = [[NSDate alloc] init];
...
{
    [(NSMutableArray *)_childPackets addObject:packet];
}
...
- (NSData *)encode
{
    ...
    [data appendData:[OSCPacket dataForContentObject:_timetag]];
    for (OSCPacket *child in _childPackets)
    {
...
- (NSString *)description
{
    return [NSString stringWithFormat:@"<OSCMutableBundle (%lu) @ %@>", (unsigned long)[_childPackets count], _timetag];
}
